...
public class Maze {
        ...
        for (int attempts = 0; attempts < 4; attempts++) {
           ...
           switch( temp){
               ...
               case 0: //NORTH
                   ...
                   this.current_row=row_nr-1; this.current_col=col_nr; return true;} break;
               case 1: //EST
               ...
               case 3: //WEST
                   ...
                   {System.out.println( this.schita[row_nr][col_nr-1].isPath );
                       ...
                       this.current_row=row_nr; this.current_col=col_nr-1; return true;} break;
           }
    ...
    public boolean checkIfPerfect(int entrance_row, int entrance_col,
                                  int exit_row,     int exit_col) {
        boolean[][] visited = new boolean[nr_rows][nr_cols];
        Deque<int[]> queue = new ArrayDeque<>();
        queue.add(new int[]{entrance_row, entrance_col});
        ...
        while (!queue.isEmpty()) {
            ...
            if (r == exit_row && c == exit_col) {
                System.out.println("Maze-ul ESTE CORECT! De la (" +
                        entrance_row + "," + entrance_col + ") la (" +
                        exit_row + "," + exit_col + ").");
                return true;
            }
            Cell cell = schita[r][c];
            if (!cell.wall_N && r > 0 && !visited[r-1][c]) { visited[r-1][c]=true; queue.add(new int[]{r-1,c}); }
            if (!cell.wall_S && r < nr_rows-1 && !visited[r+1][c]) { visited[r+1][c]=true; queue.add(new int[]{r+1,c}); }
            if (!cell.wall_W && c >0 && !visited[r][c-1]) { visited[r][c-1]=true; queue.add(new int[]{r,c-1}); }
            if (!cell.wall_E && c < nr_cols-1 && !visited[r][c+1]) { visited[r][c+1]=true; queue.add(new int[]{r,c+1}); }
        }

        System.out.println("NU ESTE BUM (" +
                entrance_row + "," + entrance_col + ") to (" +
                exit_row + "," + exit_col + ").");
        return false;
    }
}
