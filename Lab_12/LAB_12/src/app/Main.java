...
public class Main {
    ...
    static List<Class<?>> adnotariGasite = new ArrayList<>();
    ...
    static void proceseazaFisierSingur(File fisier) throws Exception {
        ...
        if (!fisierClass.getName().endsWith(".class")) {
            System.out.println("Tip de fisier necunoscut.");
            return;
        }
    ...
    static void proceseazaFolder(File folder) throws Exception {
        ...
        File dirOut = new File(folder, "_out");
        dirOut.mkdirs();
        ...
        for (File f : toateFisierele) {
            ...
            else if (f.getName().endsWith(".class") && !f.getAbsolutePath().contains("_out")) {
                claseDinDisk.add(f);}
        }
    ...
    static void afiseazaPrototip(Class<?> clasa) {
        System.out.println("\n--- " + Modifier.toString(clasa.getModifiers()) + " class " + clasa.getSimpleName() + " ---");
        for (Constructor<?> c : clasa.getDeclaredConstructors()) {
            System.out.println("  constructor: " + clasa.getSimpleName()
                    + "(" + parametriSir(c.getParameterTypes()) + ")");
        }
            ...
            for (Class<?> tipAnn : adnotariGasite) {
                ...
                try {
                    if (params.length == 0) {
                        ...
                        System.out.println("  [LOG] Inainte de executie: " + clasa.getSimpleName() + "." + m.getName() + "(42)");
    ...
    static File compileaza(File sursa, File dirIesire) {
        ...
        if (compilator == null) {
            System.out.println("JDK nu are compilator disponibil.");
            return null;
        }
        int cod = compilator.run(null, null, null,
                "-d", dirIesire.getAbsolutePath(),
                sursa.getAbsolutePath());
        if (cod != 0) {
            ...
            try {
                ...
                for (int i = 1; i < cpCount; i++) {
                    ...
                    if (tag == 1) {
                        ...
                        dis.readUnsignedByte(); dis.readUnsignedShort();
                    }
                }
                dis.close();
                for (String s : utfEntries) {
                    if (s.contains("/") && !s.contains("(") && !s.startsWith("java")) {
                        String pachet = s.replace("/", File.separator);
                        String caleAbs = fc.getAbsolutePath();
                        int idx = caleAbs.lastIndexOf(pachet);
                        if (idx != -1) {
                            return new File(caleAbs.substring(0, idx));
                        }
                    }
                }
            } catch (Exception e) {
                System.out.println("[gasesteRadacina eroare] " + e.getMessage());
            }
        }
        return folder;
    }
    ...
    static void colecteazaRecursiv(File dir, List<File> lista) {
        ...
        for (File f : continut) {
            if (f.isDirectory()) colecteazaRecursiv(f, lista);
            else lista.add(f);
           }
    }
}